// Волна 122 (пилот видео-hero): сжатие скачанного оригинала фонового видео hero
// до веб-разумного размера + извлечение постера. По образцу optimize-park-images.mjs
// (src/out явно раздельные пути, до/после в консоли), но для видео вместо sharp
// используется ffmpeg-static — единственный портируемый способ дать H.264-кодек
// без системной зависимости от ffmpeg на машине разработчика (Windows/mac/Linux).
//
// ВАЖНО: исходник — НЕ в public/. Раздавать посетителям сырой файл 6–13 МБ с
...
// Источник (автор/лицензия/ссылка) см. content-plan.md, «Волна 122».
...
const jobs = [
  {
    ...
    // Не выше 1280px по широкой стороне (ТЗ) — исходники кандидатов 1920x1080,
